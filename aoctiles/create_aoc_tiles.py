...
def format_time(time: str) -> str:
    ...
    if ">" in time:
        ...
        h, m, s = time.split(":")
        formatted = f">{h}h" if int(h) >= 1 else f"{m:2}:{s:2}"
...
def generate_day_tile_image(day: str, year: str, languages: list[str], day_scores: typing.Union[DayScores , None]) -> Path:
...
def handle_day(day: int, year: int, solutions: list[str], html: HTML, day_scores: typing.Union[DayScores , None]):
    languages = []
